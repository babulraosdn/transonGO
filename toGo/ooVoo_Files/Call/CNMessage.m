...
+(NSString *) buildMessage:(CNMessageType) type confId:(NSString *) confId to:(NSArray *) to name:(NSString *) name userData:(NSString*) extra{
    ...
    [dictionary setValue:ooVooAppID forKey:UNIQUE_ID];
...
- (instancetype) initMessageWithResponse:(ooVooMessage *) response{
    ...
    NSString * acceptedUser = [dictIs objectForKey:@"display name"];
    if ([[dictIs objectForKey:@"type"] isEqualToString:@"accept"]) {
        
        [appDelegate.callingUsers removeObject:acceptedUser];
        
        for (NSString *remainigUser in appDelegate.callingUsers) {
        ...
        for (int i=0; i<3; i++) {
            if (i==0) {
                NSArray *arrFriends = [NSArray arrayWithObject:@"smart123"];
            ...
            if (i==1) {
                NSArray *arrFriends = [NSArray arrayWithObject:@"ravi123"];
                [[MessageManager sharedMessage]messageOtherUsers:arrFriends WithMessageType:Cancel WithConfID:[ActiveUserManager activeUser].randomConference Compelition:^(BOOL CallSuccess) {
                    
                }];
            }
            if (i==2){
                NSArray *arrFriends = [NSArray arrayWithObject:@"nilesh123"];
                [[MessageManager sharedMessage]messageOtherUsers:arrFriends WithMessageType:Cancel WithConfID:[ActiveUserManager activeUser].randomConference Compelition:^(BOOL CallSuccess) {
